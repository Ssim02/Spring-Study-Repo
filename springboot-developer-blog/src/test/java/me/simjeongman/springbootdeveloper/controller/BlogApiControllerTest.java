...
class BlogApiControllerTest {
    ...
    public void deleteArticle() throws Exception {
        ...
        // when : 저장한 블로그 글의 id값으로 삭제 API를 호출
        mockMvc.perform(delete(url, savedArticle.getId()))
                .andExpect(status().isOk());

        // then : 응답 코드가 200 OK 이고, 블로그 글 리스트를 전체 조회해 조회한 배열 크기가 0인지를 확인
        List<Article> articles = blogRepository.findAll();
    ...
    public void updateArticle() throws Exception {
        ...
        // when : UPDATE API로 수정 요청을 보냄. 이때 요청타입은 JSON이고, given 절에서 미리 만들어둔 객체를 요청 본문으로 함께 보낸다.
        ResultActions resultActions = mockMvc.perform(put(url, savedArticle.getId())
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .content(objectMapper.writeValueAsString(request)));

        // then : 응답 코드가 200 OK 인지를 확인. 블로그 글 id로 조회한 후에 값이 수정되었는지를 확인한다.
